...
class SoldRow:
    ...
    search_result_item_url: str = ""
    result_position_on_page: str = ""
    search_page_number: str = ""
    search_keywords: str = ""
...
def extract_rows_from_page(page: Page) -> List[SoldRow]:
        ...
        () => {
            ...
            for (const card of cards) {
                ...
                rows.push({
                    ...
                    item_id: itemId,
                    search_result_item_url: href,
                    result_position_on_page: String(resultPosition)
                });
            }
    ...
    for r in raw_rows:
        ...
        item_id = normalize_ws(r.get("item_id")) or extract_item_id(url)
        search_result_item_url = normalize_ws(r.get("search_result_item_url")) or url
        result_position_on_page = normalize_ws(r.get("result_position_on_page"))
        ...
        rows.append(
            SoldRow(
                ...
                image_url=image_url,
                item_id=item_id,
                search_result_item_url=search_result_item_url,
...
def scrape_all_pages(
    ...
    default_sort_order = get_query_param(search_url, "_sop") or "13"

    for page_num in range(1, max_pages + 1):
        goto_page(page, search_url, page_num)
        wait_for_results(page)

        current_results_url = normalize_ws(page.url) or (
            f"{search_url}{'&' if '?' in search_url else '?'}_pgn={page_num}"
            if page_num > 1
            else search_url
        )

        current_sort_order = get_query_param(current_results_url, "_sop") or default_sort_order

        rows = extract_rows_from_page(page)

        added = 0
        for row in rows:
            row.item_id = row.item_id or extract_item_id(row.item_url)
            row.search_results_url = current_results_url
            row.search_result_item_url = row.search_result_item_url or row.item_url
            row.search_page_number = str(page_num)
            row.search_keywords = search_keywords
            row.search_sort_order = current_sort_order

            key = row.item_id or extract_item_id(row.item_url) or row.item_url
            if key not in seen:
                seen.add(key)
                all_rows.append(row)
                added += 1
...
def main() -> None:
    ...
    metadata_by_item_id: dict[str, dict[str, str]] = {}
    metadata_by_item_url: dict[str, dict[str, str]] = {}

    for row in raw_dict_rows:
        item_url = normalize_ws(str(row.get("item_url") or ""))
        item_id = normalize_ws(str(row.get("item_id") or "")) or extract_item_id(item_url)

        metadata = {
            "image_url": normalize_ws(str(row.get("image_url") or "")),
            "item_id": item_id,
            "search_results_url": normalize_ws(str(row.get("search_results_url") or "")),
            "search_result_item_url": normalize_ws(str(row.get("search_result_item_url") or "")),
            "result_position_on_page": normalize_ws(str(row.get("result_position_on_page") or "")),
            "search_page_number": normalize_ws(str(row.get("search_page_number") or "")),
            "search_keywords": normalize_ws(str(row.get("search_keywords") or "")),
            "search_sort_order": normalize_ws(str(row.get("search_sort_order") or "")),
        }

        if item_id and item_id not in metadata_by_item_id:
            metadata_by_item_id[item_id] = metadata

        if item_url and item_url not in metadata_by_item_url:
            metadata_by_item_url[item_url] = metadata
    ...
    for row in normalized_rows:
        ...
        metadata = {}
        if item_id and item_id in metadata_by_item_id:
            metadata = metadata_by_item_id[item_id]
        elif item_url and item_url in metadata_by_item_url:
            metadata = metadata_by_item_url[item_url]

        row["image_url"] = metadata.get("image_url", "")
        row["item_id"] = metadata.get("item_id", item_id)
        row["search_results_url"] = metadata.get("search_results_url", "")
        row["search_result_item_url"] = metadata.get("search_result_item_url", "")
        row["result_position_on_page"] = metadata.get("result_position_on_page", "")
        row["search_page_number"] = metadata.get("search_page_number", "")
        row["search_keywords"] = metadata.get("search_keywords", "")
        row["search_sort_order"] = metadata.get("search_sort_order", "")
